...
function BookingPage() {
  ...
  // Reverted to direct booking confirmation logic
  const handleConfirmBooking = async () => {
    ...
    setError(null);
    try {
      // Step 1: Insert the booking directly into the database
      const { error: bookingError } = await supabase.from("bookings").insert({
        ...
        status: "confirmed",
        payment_status: "paid", // Mark as 'paid' for now
      });

      if (bookingError) throw bookingError;

      // Step 2: Mark the time slot as unavailable
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <h1 className="text-center text-3xl font-bold mb-8 text-dark-text">
      ...
      <div className="max-w-lg mx-auto bg-card-bg p-8 rounded-xl border border-border-color shadow-lg">
        ...
        <p className="text-medium-text mb-6">
          <strong>Facility:</strong> {facility.name} ({facility.sports.name})
        </p>
        <div className="space-y-4 text-medium-text border-t border-b border-border-color py-6">
          <p>
            <strong>Date:</strong> {formatDate(slot.start_time)}
          </p>
          <p>
            <strong>Time:</strong> {formatTime(slot.start_time)} -{" "}
        ...
        <button
          ...
          className="w-full mt-8 py-4 px-6 rounded-lg font-semibold text-lg transition duration-300 bg-primary-green text-white shadow-sm hover:bg-primary-green-dark hover:-translate-y-px hover:shadow-md disabled:bg-gray-400"
          disabled={loading}
        >
