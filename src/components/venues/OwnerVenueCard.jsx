...
function OwnerVenueCard({ venue }) {
  ...
  const isRestricted = !isApproved && venue.rejection_reason;

  const statusConfig = isApproved
    ? { label: 'Approved', className: 'bg-light-green-bg text-emerald-800 border-green-200' }
    : isRestricted
    ...
    <div className="bg-card-bg border border-border-color rounded-xl overflow-hidden transition duration-300 shadow-sm">
      ...
      <div className="p-6">
        ...
        <div className="mt-4 pt-4 border-t border-border-color flex justify-between items-center">
          <span className={`py-1 px-4 rounded-full text-xs font-bold uppercase tracking-wider border ${statusConfig.className}`}>
            {statusConfig.label}
          </span>
